######FROM HERE ON OUT Darvish
# Print the first 6 rows of ff_dt
head(sl_dt_YD)
#format game dates in this vector
# Create game_date in ff_dt
sl_dt_YD$game_date <- as.Date(row.names(sl_dt_YD), "%Y-%m-%d")

# Rename the first column
colnames(sl_dt_YD)[1] <- "SL_spin_rate"

# Remove row names
row.names(sl_dt_YD) <- NULL

# View head of ff_dt
head(sl_dt_YD)
#plot game by game slider spin rate
plot(sl_dt_YD$SL_spin_rate ~ sl_dt_YD$game_date, lwd = 4, type = "l", ylim = c(1500, 2800),
     main = "CK Slider Spin Rate",
     xlab = "Date", ylab = "Spin Rate")
# Add jittered points to the plot
points(AllFX17_YD_SL$spin_rate ~ jitter(as.numeric(AllFX17_YD_SL$game_date)),
       pch = 16, col = "#99004450")
#PITCH MIX table
# Subset the data to remove pitch types "IN" and "EP"
AllFX17_YD <- subset(AllFX17_YD, pitch_type != "IN" & pitch_type != "EP")

# Drop the levels from pitch_type
#AllFX17_YD$pitch_type <- droplevels(AllFX17_YD$pitch_type)
#character class, rather than factor so above function doesn't do anything
# Create type_tab
type_tab <- table(AllFX17_YD$pitch_type, AllFX17_YD$PS_All)
...
SL_sr_month <- tapply(AllFX17_YD$spin_rate, AllFX17_YD$PS_All, mean)
...
AllFX17_YD$late_in_game <- ifelse(AllFX17_YD$inning > 5, 1, 0)
# Convert late_in_game
AllFX17_YD$late_in_game <- factor(AllFX17_YD$late_in_game)
# Create type_late
type_late_prop <- table(AllFX17_YD$pitch_type, AllFX17_YD$late_in_game)
...
AllFX17_YD$men_on_base <- ifelse(complete.cases(AllFX17_YD$on_1b), 1, 0)
AllFX17_YD$men_on_base[complete.cases(AllFX17_YD$on_2b) & !complete.cases(AllFX17_YD$on_3b)] <- 2
AllFX17_YD$men_on_base[complete.cases(AllFX17_YD$on_3b)] <- 3
AllFX17_YD$men_on_base <- factor(AllFX17_YD$men_on_base)

#AllFX17_YD$men_on_base[complete.cases(AllFX17_YD$on_1b) & complete.cases(AllFX17_YD$on_2b)] <- '1st and 2nd'
#AllFX17_YD$men_on_base[complete.cases(AllFX17_YD$on_3b) & complete.cases(AllFX17_YD$on_2b)] <-
#                                      "2nd and 3nd"
#AllFX17_YD$men_on_base[complete.cases(AllFX17_YD$on_1b) & complete.cases(AllFX17_YD$on_3b)] <-
#                                                             "1st and 3rd"
#AllFX17_YD$men_on_base[complete.cases(AllFX17_YD$on_3b) & complete.cases(AllFX17_YD$on_2b)
#                                                             & complete.cases(AllFX17_YD$on_1b)] <- "Bases Loaded"
#AllFX17_YD$men_on_base[AllFX17_YD$men_on_base == 0] <- 'Bases Empty'
unique(AllFX17_YD$men_on_base)
#AllFX17_YD$man_on_3rd <- ifelse(complete.cases(AllFX17_YD$on_3b), 1, 0)
#AllFX17_YD$man_on_1st <- factor(AllFX17_YD$man_on_1st)
#AllFX17_YD$man_on_2nd <- factor(AllFX17_YD$man_on_2nd)
#AllFX17_YD$man_on_3rd <- factor(AllFX17_YD$man_on_3rd)
type_OB <- table(AllFX17_YD$pitch_type, AllFX17_YD$men_on_base)
type_OB
...
tapply(AllFX17_YD$pz, AllFX17_YD$PS_All, mean) * 12
# Create AllFX17_YD_lhb
AllFX17_YD_lhb <- subset(AllFX17_YD, batter_stand == "L")
# Create AllFX17_YD_rhb
AllFX17_YD_rhb <- subset(AllFX17_YD, batter_stand == "R")
# Compute average px location for LHB
tapply(AllFX17_YD_lhb$px, AllFX17_YD_lhb$PS_All, mean) * 12
# Compute average px location for RHB
tapply(AllFX17_YD_rhb$px, AllFX17_YD_rhb$PS_All, mean) * 12

# Plot location of all pitches
plot(AllFX17_YD$pz ~ AllFX17_YD$px,
     col = factor(AllFX17_YD_lhb$PS_All),
     xlim = c(-3, 3))
# Formatting code, don't change this
par(mfrow = c(1, 2))
# Plot the pitch loctions for July
plot(pz ~ px, data = AllFX17_YD[AllFX17_YD$PS_All != "other"],
     col = "red", pch = 16,
...
# Create greinke_sub
AllFX17_YD$zone_px <- (ceiling(AllFX17_YD$px) + floor(AllFX17_YD$px)) / 2
AllFX17_YD$zone_pz <- (ceiling(AllFX17_YD$pz) + floor(AllFX17_YD$pz)) / 2

AllFX17_YD_sub <- subset(AllFX17_YD, px>-2 & px<2 & pz>0 & pz<5)
...
# Create table
AllFX17_YD_table <- table(AllFX17_YD_sub$zone)

# Create zone_prop
zone_prop <- round(prop.table(AllFX17_YD_table), 3)
...
for(i in 1:20) {
  text(mean(AllFX17_YD_sub$zone_px[AllFX17_YD_sub$zone == i]),
       mean(AllFX17_YD_sub$zone_pz[AllFX17_YD_sub$zone == i]),
       zone_prop[i], cex = 1.5)
}
...
# Create zone_prop_PS
zone_prop_PS <- round(
  table(AllFX17_YD_sub$zone[AllFX17_YD_sub$PS_All != "other"]) /
    nrow(subset(AllFX17_YD_sub, PS_All != "other")), 3)
...
AllFX17_YD_sub$bs_count <-
  # Create greinke_zone_tab
  CK_zone_tab <- table(AllFX17_YD_sub$zone, AllFX17_YD_sub$bs_count)
...
AllFX17_YD_SL$batter_swing <- ifelse(AllFX17_YD_SL$pitch_result %in% no_swing, 0, 1)
# Create swing_SL
swing_sl <- subset(AllFX17_YD_SL, AllFX17_YD_SL$batter_swing == 1)
...
no_swing <- c("Ball", "Called Strike", "Ball In Dirt", "Hit By Pitch", "Intent Ball", "Pitchout", "Automatic Ball")
AllFX17_YD$batter_swing <- ifelse(AllFX17_YD$pitch_result %in% no_swing, 0, 1)
swings <- subset(AllFX17_YD, AllFX17_YD$batter_swing == 1)
